...
def extract_specialty_from_response(response_text, language='ar'):
    if language == 'ar':
        match = re.search(r'طبيب\s+([\u0600-\u06FF\s]+)', response_text)
        return match.group(1).strip() if match else None
    elif language == 'fr':
        match = re.search(r'médecin\s+([a-zA-Zéèêàçâîôûùëïü\s]+)', response_text, re.IGNORECASE)
        return match.group(1).strip() if match else None
    elif language == 'en':
        match = re.search(r'doctor\s+([a-zA-Z\s]+)', response_text, re.IGNORECASE)
        return match.group(1).strip() if match else None
    return None
...
def diagnose():
    try:
        ...
        if session_data['step'] == len(step_questions):
            full_info = "\n".join(
                f"{step_questions[i]} {session_data['answers'][i]}"
                for i in range(len(step_questions))
            )
